// ========================================
// Firebase 설정 (auth.js와 동일하게 입력)
// ========================================

/**
 * Firebase Console에서 설정 정보를 가져와 아래에 입력하세요
 * auth.js 파일의 firebaseConfig와 동일한 값을 입력해야 합니다!
 * 
 * 1. Firebase Console 접속: https://console.firebase.google.com
 * 2. 프로젝트 설정 > 일반 > 내 앱 > 웹 앱
 * 3. firebaseConfig 복사/붙여넣기
 */
...
async function loadFromFirebase() {
    ...
    try {
        ...
        console.log(`✅ Firebase에서 ${transactions.length}개 거래 불러옴 (사용자: ${currentUser.email})`);
    } catch (error) {
        console.error('❌ Firebase 불러오기 오류:', error);
        // Firebase 실패 시 로컬스토리지에서 불러오기 시도
        console.log('⚠️ 로컬스토리지에서 데이터 불러오기 시도');
        const saved = localStorage.getItem(`overseasTransactions_${currentUser?.uid || 'local'}`);
        if (saved) {
